...
def _require_point_in_time_eligibility(research_snapshot: dict) -> None:
    point_in_time = research_snapshot.get("point_in_time")
    if (
        not isinstance(point_in_time, dict)
        or point_in_time.get("prospective_decision_eligible") is not True
    ):
        raise _conflict_error(
            error="research_snapshot_not_point_in_time_eligible",
            message=(
                "Portfolio research snapshot is not eligible for a prospective decision; "
                "refresh preflight after resolving point-in-time blockers"
            ),
        )
...
def build_research_baseline(
    ...
    try:
        ...
        _require_point_in_time_eligibility(research_snapshot)
