...
class PortfolioPointInTimeEligibility(BaseModel):
    scope: Literal["current_prospective"]
    prospective_decision_eligible: bool
    historical_replay_eligible: bool
    source_cutoffs: Dict[str, Optional[str]] = Field(default_factory=dict)
...
class PortfolioResearchSnapshotResponse(BaseModel):
    ...
    point_in_time: PortfolioPointInTimeEligibility
    decision_signals: List[Dict[str, Any]] = Field(default_factory=list)
...
class PortfolioResearchBaselineItem(BaseModel):
    ...
    user_instruction: Literal["add", "hold", "reduce", "exit", "insufficient"]
