...
class TestStorage(unittest.TestCase):
    ...
    def test_decision_quality_sidecar_tables_and_unique_keys_are_created_idempotently(self):
        ...
        try:
            ...
            self.assertIn(["material_event_fingerprint"], context_unique_keys)
            self.assertIn(["signal_id", "horizon", "engine_version"], outcome_unique_keys)
    ...
    def test_decision_quality_context_migration_adds_instrument_type_and_preserves_rows(self):
        DatabaseManager.reset_instance()
        temp_dir = tempfile.TemporaryDirectory()
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                conn.execute(
                    "INSERT INTO decision_signal_quality_contexts (signal_id, stock_code) "
                    "VALUES (7, 'AAPL')"
                )
            ...
            self.assertEqual(row, (7, "AAPL", None))
        finally:
            DatabaseManager.reset_instance()
            Config.reset_instance()
            temp_dir.cleanup()
