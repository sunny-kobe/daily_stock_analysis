...
def test_freeze_context_preserves_refresh_audit_with_same_sample_identity(isolated_db) -> None:
    ...
    assert repeated["signal_id"] == 102
    assert repeated["material_event_fingerprint"] == first["material_event_fingerprint"]
...
def test_quiet_snapshot_refresh_preserves_audit_without_new_sample_identity(isolated_db) -> None:
    service = DecisionQualityService(repo=DecisionQualityRepository(isolated_db))
    first = service.freeze_context(
        signal=_signal(101, market_phase="postmarket"),
        portfolio_decision=_decision(strategy_version="champion-v1"),
        frozen_snapshot=_snapshot(
            snapshot_hash="a" * 64,
            cutoff="2026-07-25T08:00:00Z",
        ),
    ...
    repeated = service.freeze_context(
        signal=_signal(102, market_phase="postmarket"),
        portfolio_decision=_decision(strategy_version="champion-v1"),
        frozen_snapshot=_snapshot(
            snapshot_hash="b" * 64,
