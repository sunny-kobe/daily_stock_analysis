...
def test_evaluates_exact_forward_trading_bars_and_persists_metrics(
    ...
    stock_return = ((100.0 + days) / 101.0 - 1.0) * 100.0
    benchmark_return = ((200.0 + days) / 201.0 - 1.0) * 100.0
    ...
    assert result["max_favorable_excursion_pct"] == pytest.approx(
        ((102.0 + days) / 101.0 - 1.0) * 100.0
    )
    assert result["max_adverse_excursion_pct"] == pytest.approx((98.0 / 101.0 - 1.0) * 100.0)
    assert result["instrument_type"] == "equity"
    persisted = DecisionQualityRepository(isolated_db).list_quality_outcomes(
        signal_id=101, horizon=horizon, engine_version="decision-quality-v1"
    )
...
def test_hold_and_exit_normalized_action_returns(isolated_db) -> None:
    ...
    hold_return = (105.0 / 101.0 - 1.0) * 100.0
    assert hold["normalized_action_return_pct"] == pytest.approx(hold_return)
    ...
    assert exited["decision_value_vs_hold_pct"] == pytest.approx(-hold_return)
    assert exited["hindsight_regret_pct"] == pytest.approx(hold_return)
...
def test_uses_first_tradable_open_after_cutoff_instead_of_same_day_close(isolated_db) -> None:
    _context(isolated_db, decision_cutoff=datetime(2026, 1, 2, 12, 0))
    _seed_series(
        isolated_db,
        code="AAPL",
        start=date(2026, 1, 2),
        closes=[100.0, 110.0, 111.0, 112.0, 113.0, 114.0],
    )
    _seed_series(
        isolated_db,
        code="SPY",
        start=date(2026, 1, 2),
        closes=[200.0, 210.0, 211.0, 212.0, 213.0, 214.0],
    )
    ...
    assert result["eval_status"] == "complete"
    assert result["anchor_date"] == date(2026, 1, 3)
    assert result["start_price"] == 110.0
    assert result["end_close"] == 114.0


def test_completed_service_outcome_ignores_later_market_data_revisions(isolated_db) -> None:
    _context(isolated_db)
    _seed_paired_bars(isolated_db, count=6)
    service = DecisionQualityService(db_manager=isolated_db)

    first = service.evaluate_outcome(signal_id=101, horizon="5d")
    with isolated_db.session_scope() as session:
        revised = (
            session.query(StockDaily)
            .filter(StockDaily.code == "AAPL", StockDaily.date == date(2026, 1, 7))
            .one()
        )
        revised.close = 500.0
        revised.high = 500.0

    repeated = service.evaluate_outcome(signal_id=101, horizon="5d")

    assert first["eval_status"] == "complete"
    assert repeated["end_close"] == first["end_close"]
    assert repeated["stock_return_pct"] == first["stock_return_pct"]
