...
def _outcome_fields(**overrides):
    fields = {
        ...
        "data_revision_hash": "d" * 64,
        "input_bar_hash": "e" * 64,
        "computed_at": datetime(2026, 7, 30, 8, 0),
        "eval_status": "pending",
        "unable_reason": None,
        "anchor_date": None,
...
def test_context_creation_preserves_each_signal_but_reuses_sample_identity(isolated_db) -> None:
    ...
    assert repeated.signal_id == 202
    assert repeated.material_event_fingerprint == created.material_event_fingerprint
    assert repo.get_context_by_signal(signal_id=101).id == created.id
    assert repo.get_context_by_signal(signal_id=202).id == repeated.id
...
def test_quality_outcomes_remain_separate_across_horizons_and_versions(isolated_db) -> None:
    ...
    repo.upsert_quality_outcome(
        _outcome_fields(horizon="20d", eval_window_days=20, data_revision_hash="f" * 64)
    )
    repo.upsert_quality_outcome(
        _outcome_fields(engine_version="decision-quality-v2", data_revision_hash="1" * 64)
    )
...
def test_completed_outcome_is_immutable_within_engine_and_data_revision(isolated_db) -> None:
    repo = DecisionQualityRepository(isolated_db)
    complete = _outcome_fields(
        eval_status="complete",
        start_price=100.0,
        end_close=105.0,
        stock_return_pct=5.0,
    ...
    repo.upsert_quality_outcome(complete)

    with pytest.raises(ValueError, match="completed_outcome_immutable"):
        repo.upsert_quality_outcome({**complete, "end_close": 106.0, "stock_return_pct": 6.0})

    persisted = repo.list_quality_outcomes(signal_id=101)
    assert len(persisted) == 1
    assert persisted[0].end_close == 105.0
