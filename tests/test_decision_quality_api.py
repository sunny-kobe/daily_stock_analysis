...
def _seed_material_decision(db, *, bar_count=6):
    ...
    DecisionQualityRepository(db).create_context_if_absent(
        {
            ...
            "frozen_position_quantity": 100.0,
...
def test_quality_stats_exposes_sample_size_and_instrument_concentration(client_and_db):
    ...
    payload = client.get("/api/v1/decision-signals/quality/stats").json()
...
def test_quality_stats_never_pool_distinct_horizons(client_and_db):
    client, db = client_and_db
    signal_id = _seed_material_decision(db, bar_count=21)
    run = client.post(
        "/api/v1/decision-signals/quality/outcomes/run",
        json={"signal_id": signal_id, "horizons": ["5d", "20d"]},
    )
    ...
    assert {(group["horizon"], group["event_count"]) for group in payload["groups"]} == {
        ("5d", 1),
        ("20d", 1),
    }
    assert all(group["strategy_version"] == "champion-v1" for group in payload["groups"])
...
def test_execution_link_api_attributes_trade_without_rewriting_it(client_and_db):
    client, db = client_and_db
    signal_id = _seed_material_decision(db)
    with db.session_scope() as session:
        session.add(
    ...
    response = client.put(
        f"/api/v1/decision-signals/{signal_id}/execution-links/{trade_id}",
        json={
            "link_status": "confirmed",
            "linked_by": "human",
            "note": "Confirmed after the frozen signal.",
        },
    )
    listed = client.get(f"/api/v1/decision-signals/{signal_id}/execution-links")

    assert response.status_code == 200, response.text
    assert response.json()["actual_position_action"] == "reduce"
    assert listed.status_code == 200, listed.text
    assert listed.json()["links"][0]["trade_id"] == trade_id
    with db.get_session() as session:
        persisted = session.get(PortfolioTrade, trade_id)
        assert persisted.quantity == 40.0
        assert persisted.price == 101.0
        assert persisted.fee == 1.0
        assert persisted.tax == 0.2
