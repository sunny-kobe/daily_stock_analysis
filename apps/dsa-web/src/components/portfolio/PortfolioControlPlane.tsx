...
export function PortfolioControlPlane({ onInstrumentsLoaded }: PortfolioControlPlaneProps) {
  ...
  const [pointInTime, setPointInTime] = useState<PortfolioPointInTimeEligibility | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      ...
      setPointInTime(snapshot.pointInTime || null);
      if (policy.policy) {
        setPolicyForm({
          ...
          {pointInTime ? (
            <div className="pb-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h3 className="text-xs font-semibold text-foreground">{t('portfolio.controlPlane.eligibilityTitle')}</h3>
                <code className="text-[11px] text-secondary">{pointInTime.scope}</code>
              </div>
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs">
                <span className={pointInTime.prospectiveDecisionEligible ? 'text-success' : 'text-warning'}>
                  {pointInTime.prospectiveDecisionEligible
                    ? t('portfolio.controlPlane.prospectiveReady')
                    : t('portfolio.controlPlane.prospectiveNotReady')}
                </span>
                <span className="text-secondary">{t('portfolio.controlPlane.historicalUnavailable')}</span>
              </div>
              <p className="mt-3 text-[11px] font-medium text-secondary">{t('portfolio.controlPlane.sourceCutoffs')}</p>
              <dl className="mt-1 grid grid-cols-1 gap-x-3 gap-y-1 text-[11px] sm:grid-cols-2">
                {Object.entries(pointInTime.sourceCutoffs).sort(([left], [right]) => left.localeCompare(right)).map(([source, cutoff]) => (
                  <div key={source} className="flex min-w-0 items-baseline justify-between gap-2">
                    <dt className="font-mono text-secondary">{source}</dt>
                    <dd className="min-w-0 truncate text-right text-foreground" title={cutoff || undefined}>
                      {cutoff || t('portfolio.controlPlane.noSourceCutoff')}
                    </dd>
                  </div>
                ))}
              </dl>
              {pointInTime.blockers.length > 0 ? (
                <div className="mt-2 flex flex-wrap gap-1.5" aria-label={t('portfolio.controlPlane.temporalBlockers')}>
                  {pointInTime.blockers.map((code) => (
                    <span key={code} className="rounded border border-amber-400/30 px-2 py-1 text-[11px] text-warning">{code}</span>
                  ))}
                </div>
              ) : null}
            </div>
          ) : null}
          <div className={pointInTime ? 'border-t border-white/10 pt-3' : ''}>
