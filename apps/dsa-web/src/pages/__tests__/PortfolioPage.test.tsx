...
const {
  ...
  getStrategyValidationReviewSummary,
} = vi.hoisted(() => ({
  getAccounts: vi.fn(),
  getSnapshot: vi.fn(),
...
describe('PortfolioPage FX refresh', () => {
  beforeEach(() => {
    ...
    getStrategyValidationReviewSummary.mockResolvedValue({
      strategyId: 'portfolio-champion',
      protocolId: null,
      champion: { strategyVersion: null },
      challenger: { strategyVersion: null },
      historicalOos: { status: 'unable' },
      prospectiveShadow: { status: 'unable' },
      hardGateFailures: [],
      sampleConcentration: { status: 'unable' },
      costDeltaPct: null,
      drawdownDeltaPct: null,
      unableReasons: ['validation_evidence_not_recorded'],
      matureHorizons: [],
      maturityDecision: 'INSUFFICIENT_EVIDENCE',
      rollbackTarget: null,
