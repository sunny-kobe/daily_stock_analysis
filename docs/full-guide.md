...
| `/api/v1/decision-signals/{signal_id}/execution-links/{trade_id}` | PUT | 人工确认冻结建议与既有 DSA trade 的 attribution；不改账本 |
| `/api/v1/decision-signals/{signal_id}/execution-links` | GET | 查询已确认/待确认 execution link 与派生实际双轴动作 |
| `/api/v1/decision-signals/strategy-validation/review-summary` | GET | 查询 OOS、shadow、hard gate、集中度、成本、回撤与成熟周期的只读摘要 |
...
- 持仓页另有只读“策略验证复盘”面板，展示 champion/challenger、历史 OOS、prospective shadow、hard gates、样本集中度、成本/回撤差异、unable 原因、成熟周期和 rollback target；不提供批准、回滚执行、自动激活或订单控件。

完整契约、归因枚举和 shadow 启动条件见 [DecisionSignal 决策信号专题](decision-signals.md)、[DSA 组合研究控制面](portfolio-research-workflow.md) 与 [组合策略验证闭环](portfolio-strategy-validation.md)。
