...
2026-08-03 已完成固定 benchmark、复权一致性和冻结分析只读保护，但真实复跑仍为 `ready=0 / insufficient=17`：旧 `stock_daily` 的单版本结构无法同时保存同日期的旧缓存和新来源修订。下一切片先建立只追加的行情证据版本，再重跑；在产生资料完整的新建议之前，策略成绩仍没有新增有效样本。
