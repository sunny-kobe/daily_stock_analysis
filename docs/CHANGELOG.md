...
## [Unreleased]
- [文档] 修正个人投资学习顺序，前置资金约束与投资政策，并新增可复用的学习启动 Prompt。
...
- [文档] 记录首次真实全持仓模拟的失败关闭结果：17 条持仓均因重叠日线冲突或固定基准缺失显示“资料不足”，未深挖、未保存新建议、未写人工反馈或产生订单。
- [修复] 固定三地评价基准及复权身份，阻止复权口径不一致的建议，并确保冻结分析不改写行情缓存；真实 17 条持仓复跑继续对旧行情冲突失败关闭，未生成无证据建议。
