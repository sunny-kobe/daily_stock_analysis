...
2. `POST /api/v1/portfolio/research-evidence/prepare`、`GET /api/v1/portfolio/research-snapshot` 和 `POST /api/v1/portfolio/research-baseline` 必须携带同一 timezone-aware cutoff 与 scope。canonical runner 与 Web 在 prepare 请求中设置 `establish_cutoff=true`：服务端先按 `(market, symbol)` 去重捕获 QDII 的产品 quote、IOPV/reference、实时 FX 和 daily-reset 产品 quote，再建立最终 cutoff；最终值只能位于请求 cutoff 与 `prepared_at` 之间且不得跨上海自然日。预取失败也视为已尝试，后续不得在最终 cutoff 后重抓；snapshot、baseline 和 manifest 必须使用响应 cutoff。未设置该字段的旧调用继续使用严格请求 cutoff。盘前、盘中只接受上一完成交易日；市场实际收盘后可接受当天已完成 session；周末、节假日回退到上一完成交易日。数据源尚未给出应有日线时，该行保持“资料不足”，不能把昨日行情标成 fresh；snapshot 必须独立复核 position 与 benchmark bar 是否等于该 cutoff 下最新已完成 session，不能只按宽松年龄窗口重新放行 prepare 已拒绝的 bar。prepare 只做一次，覆盖行情、固定 benchmark、身份、FX 与产品约束；snapshot 一次冻结 cutoff、scope 和只读组合状态；baseline 只生成 scope 内确定性建议，不进入新闻或 LLM。
3. A 股固定基准为 Baostock 前复权 `000300`。QDII 必须具备同 cutoff 的 NAV/IOPV、溢折价、底层 FX、spread 和 tracking；其中研究阶段 tracking 可由 SSE `dayk` 中最近两个已完成 session 的产品收盘价与 IOPV 按 `product_return-reference_return` 计算，必须保存两个 session date、四个原始值、source 和 source version，并明确 `fx_incorporated_in_reference=true`。盘中未完成的当日 row 不得进入该计算，完成日 tracking 也不得替代执行时的两组 provider-timestamped 动态观测。daily-reset 产品必须具备官方条款、底层身份、路径/再平衡、流动性和完成的持有期适配评估。冻结阶段只能把产品与底层的已完成日线收益比命名为 `completed_session_leverage`，不得称为 intraday 或代替执行时同步杠杆。评估结果可以是不适配；这是风险事实而非资料缺失，但必须阻断加仓而不阻断减仓/退出。缺任一关键证据仅让该行显示“资料不足”；完整产品 fixture 必须能继续通过 baseline、保存前 gate、immutable sidecar 和 execution-check，不能在后续层重新被硬编码 blocker 关闭。冻结 instrument 的产品证据是绑定流程的权威来源，调用方 context 不得覆盖。通过 exact trace、语义身份、quality context 与 immutable evidence 校验的深研结果明确标为 `deepened`，不得继续沿用 `baseline`。
...
7. 用户接受、修改、否决或暂不行动后，按 A/H/US 各自市场窗口调用 `POST /api/v1/portfolio/research-execution-check`。每次都提交原 `research_snapshot_hash`、`research_execution_identity_hash`、cutoff、完整冻结 `research_scope` 和 baseline manifest 中未修改的 `evidence_preparation.items`，另用其严格子集 `execution_scope` 指定本次市场。若 baseline 后 API 重启，只允许用这批冻结 items 恢复相同 cutoff 的产品证据，不重新抓取旧时点；服务端要求 items 与完整 scope 精确一致，并把产品证据内容纳入稳定 execution identity，删行、串行或篡改都会在行情刷新前失败关闭。服务端随后用完整 scope 重建只读视图并拒绝现金、持仓数量、产品身份、产品证据、risk policy 或运行架构漂移，再只刷新该市场子集。旧调用不传 `execution_scope` 时只读兼容为完整 scope；同进程旧调用可继续读取内存冻结证据，但 canonical runner 和 Web 始终显式回传。行情缓存、日线快照时间和新 DecisionSignal 不会因此整批 409，逐行结果仍绑定原 snapshot hash。该路由只刷新 price、trading status、spread、volume、volume ratio、VWAP 和相关 benchmark 执行证据，不为估值、基本面或换手率等无关字段遍历补全链；Web 为每个只读长请求保留最多 5 分钟响应窗口。实时 quote 必须带 source 和 timezone-aware provider timestamp，且相对每次网络返回后重新读取的 `checked_at` 不超过 15 分钟；不能用 `fetched_at` 代替 provider timestamp。腾讯 A/H 股时间字段按交易所时区解析，港股优先实时 `r_hk*` 单票接口，Yahoo 使用 `regularMarketTime` 或带时区 history index；韩国精确底层可在 KOSPI 常规时段使用 Naver Finance 零延迟直连，但仅接受 `marketStatus=OPEN`、双重 `delayTime=0` 和带时区 `localTradedAt`，闭市更新或字段缺失继续失败关闭。provider 未给交易状态时由现有交易日历推导 regular-session 状态，日历无法判定或当前不在可执行 session 时该行失败关闭。QDII 动态证据要求 IOPV/可验证 reference value、market price、premium/discount、USD/CNY FX、spread、VWAP、tracking 输入及各自 source/provider timestamp。准备阶段冻结一组内部时间偏差不超过 2 分钟的产品价、IOPV/reference 和 FX `execution_reference_observation`，执行复核再取第二组同步观测计算区间收益；以 CNY 报价且已含汇率影响的 IOPV 用 `product_return-reference_return` 衡量 tracking，FX 收益单列而不重复扣减。SSE 完成日 tracking 只服务研究门槛，不能替代上述任一执行观测；历史 NAV、昨收、估算值、`fetched_at` 和旧 tracking 同样不能补位。daily-reset 动态证据要求产品与精确底层的 reference/current price、各自 source/provider timestamp、同步收益、`observed_leverage`、spread、volume、VWAP，产品与底层时间最大偏差 2 分钟。静态官方条款、daily reset、路径/再平衡和持有期评估继续复用冻结证据；旧 `intraday_leverage` 只能读取兼容，不能满足新的执行门槛。发生动作相关变化时要求重新确认，不重跑长期 thesis，不写 ledger、DecisionSignal、report 或 sidecar，也不输出数量和比例。
美股 daily-reset 的产品与精确底层使用 Nasdaq autocomplete 返回的 exact symbol/asset class 请求 `info` 和 `summary`，只接受 `marketStatus=Open`、`isRealTime=true`、非交叉 bid/ask 及 provider trade timestamp。Nasdaq 盘口提供 price、spread 和 volume；Yahoo regular-session 1 分钟 bars 只提供单独标记为 `one_minute_close_volume_weighted` 的 VWAP，保留独立 source/provider timestamp。两源与产品/底层任一时间偏差超过 2 分钟时该行失败关闭，不得把复合证据冒充 Nasdaq 原生 VWAP 或单一来源。

8. 5/20/60 只属于历史效果复盘，默认折叠且不请求 weekly review；用户展开后才加载。未成熟窗口不阻塞当天人工决定，资料不足的建议不计入有效策略成绩。

`captured_at` 在 portfolio market evidence 中表示绑定到最终研究 cutoff 的逻辑观测边界；网络调用实际完成时间使用 prepare response 的 `prepared_at`。provider timestamp 仍来自数据源，不能用 `captured_at`、`prepared_at` 或 `fetched_at` 代替。启用 `establish_cutoff` 时，只有最终 cutoff 建立前已捕获的动态输入可以绑定到该 cutoff；不得用 `prepared_at` 之后出现的行情或产品事实补写冻结证据。
