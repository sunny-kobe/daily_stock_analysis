...
| `REALTIME_FULL_MARKET_FALLBACK_ENABLED` | Allow efinance/Eastmoney full-market snapshots in single-symbol realtime fallback. Disabled by default to prevent a full snapshot timeout from blocking a single quote; batch prefetch remains available. | `false` | Optional |
