...
持仓分析的内部记录仍分别保存“已有仓位怎么处理”和“新增资金是否投入”：
...
两项内部记录使用独立门禁；不能从 legacy `buy/add/hold` 推断 `add_in_batches`。`ADD_IN_BATCHES` 只有在 DSA 风险预算、现金、标的身份和既有 sizing/unit contract 都充分时才可采用，否则必须是 `WAIT`/`INSUFFICIENT_EVIDENCE`，且系统不制造仓位比例或建议数量。

普通 Web 界面不会要求用户理解两项内部字段，而是投影为一个动作：`hold + add_in_batches` 显示“加仓”，`hold + wait|no_add` 显示“持有”，`reduce` 显示“减仓”，`exit` 显示“清仓”，hard blocker 显示“资料不足”。`reduce|exit + add_in_batches` 属于冲突组合并直接拒绝。用户修改单一动作时，前端会确定性映射回内部记录，原始 AI 决策仍不可改写。
